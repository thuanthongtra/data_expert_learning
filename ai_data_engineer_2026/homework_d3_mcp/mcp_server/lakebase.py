"""Lakebase helper for the weather prediction MCP server."""
...
_w = WorkspaceClient()
logger = logging.getLogger("weather-lakebase")
...
def _safe_schema_write(sql: str) -> None:
    try:
        run_write(sql)
    except psycopg2.Error as err:
        if getattr(err, "pgcode", None) == errorcodes.INSUFFICIENT_PRIVILEGE:
            logger.warning("Skipping schema DDL due to insufficient privilege: %s", err)
            return
        raise


def ensure_schema() -> None:
    _safe_schema_write("CREATE EXTENSION IF NOT EXISTS vector")
    _safe_schema_write(
        """
    ...
    _safe_schema_write("CREATE INDEX IF NOT EXISTS idx_weather_documents_location ON weather_documents (location)")
    _safe_schema_write("CREATE INDEX IF NOT EXISTS idx_weather_embeddings_document_id ON weather_embeddings (document_id)")
    _safe_schema_write(
